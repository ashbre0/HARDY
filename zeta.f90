...
program compute_hardy_fast
  ...
  integer       :: N,ip,K,icj(0:50),MIT,totblock,irsp(5,2),i,mc,m1(50),pblocks
  integer(dp1)  :: NC,NCZ,jsum,RSN,L(0:50),NC20,JE,JS,JF
  ...
  real(dp)      :: rszsumtot,rszsum1,rstime,SE
  real(dp)      :: z_inp, z_out, rz, iz, tse
  real(kind=C_DOUBLE) :: apr1,apr2,apr3,apr4,apr5,apr6
  complex(kind=16)   :: w,coe,coeff(7,0:6),zlarcoeff(-1:6),zsmalcoeff(-1:6)
  complex(kind=16)   :: c1,c2,c3,c4,epi4,errcon,qq,c5,c6,c7,cr1,cr2
  type(C_PTR)        :: apu,apv,apw1,apw2,apw,apw3,apw4,apw5,apv1,app1,app2,app3,app4,app5,aplog
  type(C_PTR)        :: aps1,aps2,aps3,aps4,aps5
  integer(kind=8)        :: N_INTERNAL, N_MAX
  real(dp)               :: elapsed_time, start_time, end_time




  COMMON/PARMS/ coeff,zlarcoeff,zsmalcoeff     ! to go
  COMMON/PARS1/ phicoeff,fracL,xr,L,icj,MIT
  COMMON/PARS2/ GAM,ZET
  COMMON/PARS4/ P,SP,TPP,TPM,EPI4              
  COMMON/PARS5/ m1
  COMMON/PARS6/ t,a,ain,et,ht,Y,K,ip,mc
  COMMON/PARS7/ see
  COMMON/PARS8/ GAMCOF
  ...
  integer(dp1)  :: M2,N1,N2,ib,ial,ae,aenums,MT,MTM
  integer(dp1)  :: aestartofblock,N1startofblock,N4
  real(dp)      :: t6,b,term,X,pcsum,e
  real(dp)      :: rmc1,rmc2,rmc3,rmc4,rmc5,rmc6,zsum1,zsum2,RN4
  real(dp)      :: tmod2pi, tlnXmod2pi, var, log_t, sqrt_t
...
!   t value stored. Now for a=sqrt(8*t/pi).
  ...
  mc=3
  ip=3
  K=400  !AB_FLAG K larger better slower
  !      et=1.0/log(1.0D18)
  et=0.005  ! error scales with et^(0.25) - double would 2^(0.25) (inverse proportion) 
  C=100.0
  Y=111/C
...
!  print*, '
!  print*, 'a before = ',a
!  print*, 'ain before = ',ain
!  print*, 'ht before = ',ht



  call pari_phases(yphase,rsphase)
  call pari_tparams(et,mc,ht,a, ain, log_t, sqrt_t)
...
!  ain=1/a
  ...
  raecutoff=exp((log(et)+mc*log_t)/(mc+1.0d0))/(sp*log_t)

  pblocks=14
!  xpbloc=(1-pblocks/log(t))/(mc+1.0)

  xpbloc=(1-pblocks/log_t)/(mc+1.0)
...
!  LC=((mc-1)*log(t)/(2*mc+2.0d0)-log(log(t))+log((et**(1/(mc+1.0d0)))/(Y*sqrt(8.0d0))))/log(F1)
  LC=((mc-1)*log_t/(2*mc+2.0d0)-log(log_t)+log((et**(1/(mc+1.0d0)))/(Y*sqrt(8.0d0))))/log(F1)
...
! aenums=11233246
  ...
  do iblock=1,168 !  168          ! totblock AB_FLAG
...
! AB_FLAG - need to reduce the time of this somehow.
! t_grabber only needs a single call...)
     ...
     CALL MPI_ALLREDUCE(zsum1,zsum2,1,MPI_REAL16,mysum,COMM,IERR)
     ...
     write(2,*) '----------------------------------------------------------------------'
        ...
        ! Store the elapsed time for this iteration
                 print*, 'Iblock = ',iblock, ' took ', elapsed_time, 'seconds'
...
!     N_MAX = 128  ! this is for testing. N_MAX should be around 20,000. Assuming this is run on 1 node (128 procs).
     ...
     DO jsum=(RANK+1),N_MAX,COMM_SIZE
        ...
        CALL pari_calc(rszsum1,jsum,rsphase,N_INTERNAL)

!             if(RANK .eq. 0) then

                 print*, 'rszsum = ', rszsum1, ' for jsum = ',jsum
...
!             endif
     ...
     if(RANK .eq. 0) then
             ...
             NCZ = (NC-(N_MAX*N_INTERNAL))
     
     CALL pari_calc(rszsum1,(N_MAX+1),rsphase,NCZ)
   
     rszsum = rszsum + rszsum1

!     print*, 'rszsum1 = ', rszsum1
     print*, '(N_MAX+1) = ', (N_MAX+1)
     print*, '(NC-(N_MAX*N_INTERNAL)) = ', NCZ
     print*, 'rszsum = ', rszsum
     ...
     print*, 'my ranks rszsum = ', rszsum, ' RANK = ',RANK

     CALL MPI_Barrier(COMM,IERR)
     CALL MPI_ALLREDUCE(RSZSUM,RSZSUM1,1,MPI_REAL16,mysum,COMM,IERR)
...
subroutine alphasum(MTM,rae,pcsum)
  ...
  integer(dp1)  :: MTM,JF
  real(dp)      :: rae,pcsum,p,sp,t,a,et,e,s,amp,s1,pc,xx,th1,th2,wm,wp,initialcoeff(3)
  real(dp)      :: rs,x0,ht,ain,Y,pc2,con1,con2,tpp,tpm,initialcoeffcc(3)
  real(dp)      :: ga1,ga2,ga3,cc,cc1,se,tse
...
subroutine phifactors(m,j)
  ...
  integer       :: m,j,icj(0:50),MIT,i,nk,n,if
  integer(dp1)  :: L(0:50)
...
subroutine computecnewton(n,nit,con1,tol,c)
  ...
  integer       :: n,nit,icj(0:50),MIT,itermax,i,j
  integer(dp1)  :: L(0:50)
...
subroutine mgausssum(m,ip,csum)
  ...
  integer       :: m,ip,MIT,icj(0:50),i,k,j,m1(50),jj
  integer(dp1)  :: L(0:50)
  real(dp)      :: initialcoeff(3),errtol,p,sp,tpp,tpm
  real(dp)      :: phicoeff(0:10,50),xr(50),fracL(0:50),x,y,rminorcor,denn
  ...
  if (L(MIT-1).eq.0.or.L(MIT-1).eq.(-1)) then
     ...
     if (MIT.gt.1) then
        rminorcor=3*phicoeff(3,MIT-1)/(xr(MIT-1)**2)
        denn=1-rminorcor*phicoeff(1,MIT-1)
     endif   
     csum=(0.0,0.0)
     do i=0,L(MIT-1)
        ...
        if (MIT.gt.1) then
           csum=csum+exp(fn)/(denn+i*rminorcor)
        else   
           csum=csum+exp(fn)
        endif   
     enddo
...
subroutine computarrs(m,initialcoeff,N,KCUT)
  ...
  integer       :: m,KCUT,icj(0:50),ix,j,jj,MIT,its,m1(50),ichange
...
!  call pari_psi(con2,ps2)
!  call pari_psi(con1,ps1)
...
!   call pari_erfc(z,cr2)


!  print*,'old calc erfc = ',cr2,'new calc erfc = ',rz,iz
...
subroutine pari_calc(rzsum,iteration,rsphase,N)
  ...
  integer*8 :: I_START, I_END, iteration, N
...
subroutine t_grabber(t)
  ...
  !       CALL pari_init(10000000_8, 2_8)
    ...
    Y = '1.8226119934463495526866993572800000000000e27'
  ! Y = '6.1550577023994937901948299472529719000000e32'  ! keep decimal digits 40 long
...
subroutine pari_erfc(z,errfunc)
  ...
  integer, parameter   :: dp = selected_real_kind(33)
  integer(kind=C_LONG) :: prec   = 3 ! 152 decimal digits
...
!  S = gatan( stor(1_8 , prec) ,prec)        ! PI/4
  ...
  u = gmul ( stoi(-1_8),u)                  ! -I
  S = gmul ( S, u      )                    ! -I*pi/4
  S = gexp ( S, prec   )                    ! exp(I*pi/4)
  S = gmul ( S, z_pari )                    ! Z*exp(I*pi/4)
...
!  CALL pari_init(10000000_8, 2_8)
  ...
  CALL real2pari(X,X_pari)   ! I THINK THIS IS A PLACE AN ERROR MIGHT POP UP

  Y_pari = gpsi(X_pari,prec)
  Y      = rtodbl(Y_pari)
...
END subroutine pari_psi


subroutine pari_phases(yphase,rsphase)
  ...
  type(C_PTR)          :: yphase_pari, theta_pari, rsphase_pari

  real(dp), INTENT(OUT):: yphase, rsphase
...
subroutine pari_tparams(et,mc,ht,a, ain, log_t, sqrt_t)
  ...
  type(C_PTR)          :: u,v,w,x,y,y1,y2
  type(C_PTR)          :: t, a_pari, ain_pari, et_pari, ht_pari
  type(C_PTR)          :: expo

  real(dp), INTENT(IN) :: et
  real(dp), INTENT(OUT):: ht, a, ain,  log_t, sqrt_t
  real(dp)             :: pi,pi2
  integer              :: mc
  real*8                  :: pi_dbl, pi2_dbl
  

  CALL pari_init(100000000_8, 2_8)


  CALL t_grabber(t)

  CALL real2pari(et,et_pari)


    v           = Pi2n(1_8, prec) ! 2pi
    w           = gsqrt( stoi(-1_8) ,prec) ! imag(1)
    y           = gdiv(v, stor(2_8,prec)) ! pi pari


    a_pari           = gmul( stor(8_8,prec) , t)
    a_pari           = gdiv( a_pari , y )
    a_pari           = gsqrt( a_pari , prec)
    ain_pari         = gdiv( stor(1_8,prec) , a_pari )

    !a       = rtodbl(a_pari)
    CALL rtoquad(a_pari,a)
    CALL rtoquad(ain_pari,ain)
!    ain     = rtodbl(ain_pari)
    ...
    ht_pari      = gdiv(u, v)
!    ht           = rtodbl(ht_pari)
    CALL rtoquad(ht_pari,ht)
    CALL rtoquad(v,log_t)
    
    v = gsqrt( t, prec )

    CALL rtoquad(v,sqrt_t)

!    num          = gmul( stor(3_8,prec) , v )                ! we want mc=3 in here really
!    num          = gadd( u , num )                           ! log(et) + mc*log(t)
!    dom          = gadd( stor(3_8,prec) , stor(1_8,prec) )   ! mc + 1



!    raecutoff_pari  = gdiv(num,dom)
!    raecutoff_pari  = gexp( raecutoff_pari , prec  )
!    dom             = gsqrt(y,prec)  ! sqrt(pi)
!    dom             = gmul(dom,v)    ! sqrt(pi)*log(t)
!    raecutoff_pari  = gdiv( raecutoff_pari , dom )


!     print*, rtodbl(raecutoff_pari)
!    CALL rtoquad(raecutoff_pari,raecutoff)
